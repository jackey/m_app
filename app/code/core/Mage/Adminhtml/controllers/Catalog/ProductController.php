...
//            }
    ...
    public function saveAction()
    {
        ...
        if ($data) {
			...
			$origin_data = array();
			$origin_gallery_id = array();
			$gallery_array = json_decode($data['product']['media_gallery']['images']);
			// Mage::log($gallery_array);
			foreach($gallery_array as $gallery){
				if(isset($gallery->value_id)){
					$origin_gallery_id[] = $gallery->value_id;
				}
			}
			
			foreach($origin_gallery_id as $_value_id){
			
				 $results = $readConnection->fetchAll("SELECT `value_id` , `description` FROM `catalog_product_entity_media_gallery_value` WHERE `value_id` = {$_value_id}");
				 $origin_data[$results[0]['value_id']] = $results[0]['description'];
			}
			// Mage::log($origin_data);
		...
		// Mage::log($origin_data);
		$media_array = json_decode($data['product']['media_gallery']['description']);
		// Mage::log($data);
		if($media_array != ""){
			foreach($media_array as $media_url){
				if($media_url->description !=""){
					$value_id = $media_url->id;
					$description = $media_url->description;
					$update_query = "UPDATE `catalog_product_entity_media_gallery_value` SET `description`= '{$description}'  WHERE `value_id` = {$value_id}";
					$writeConnection->query($update_query);
				}
			}
		
		}else{
			foreach($origin_data as $id => $description){
				$update_query = "UPDATE `catalog_product_entity_media_gallery_value` SET `description`= '{$description}'  WHERE `value_id` = {$id}";
				$writeConnection->query($update_query);
			}
